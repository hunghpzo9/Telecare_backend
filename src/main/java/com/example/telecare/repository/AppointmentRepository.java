...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer> {
    ...
    @Query(value = "SELECT a.id \n" +
            ...
            "            left outer join telecare.appointment_status aps on aps.id = ad.status_id\n" +
            "            left outer join telecare.cancel_appointment ca on ca.appointment_id = a.id\n" +
            "            where time = ?1 and ?2 >= (DATE_SUB(s.start_at, INTERVAL 900 SECOND)) " +
            "            and a.is_send_fcm_upcoming = 0 and ad.status_id = 2;",
            nativeQuery = true)
    List<AppointmentDTOInf> getComingAppointmentInFifteenMin(String date,String time);
    ...
    @Query(value = "SELECT a.id , u.id as doctorId ,p.patient_id as patientId\n" +
            ...
            "                        left outer join telecare.schedule s on a.schedule_id = s.id\n" +
            "                        left outer join telecare.appointment_status aps on aps.id = ad.status_id\n" +
            "                        where ad.status_id = 2\n" +
            "                        and time = ?1 and s.start_at <= ?2 and s.end_at >= ?2 \n" +
            "                        and a.is_send_fcm_ontime = 0"+
            "                        group by s.end_at,s.start_at,ad.time\n" +
            "                        order by ad.time",
            nativeQuery = true)
    List<AppointmentDTOInf> getCurrentAppointmentOnTime(String date, String time);
